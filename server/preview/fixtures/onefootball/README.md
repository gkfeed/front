...
The live adapter polls active candidates every 60 seconds and sweeps dormant candidates within five cycles. The BFF does not retain live match results, so manual refresh starts a fresh upstream load. Shared failed-refresh retention expires five minutes after the last successful client snapshot.

The `full-time` fixture also includes the top-level `matchEvents` container captured from the same match page on 2026-09-06. `teamSide` identifies the team credited with the goal (0 home, 1 away), including own goals. The goal enum in chunk `6663-60186cb5fd3c4ec0.js` is GOAL=0, OWN_GOAL=1, PENALTY_GOAL=2, RETRACTED_GOAL=3, MISSED_PENALTY_GOAL=4, UNRECOGNIZED=-1. Only 0–2 count toward the displayed scorers; `timeline` preserves the provider's minute text.
